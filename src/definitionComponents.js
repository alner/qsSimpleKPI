...
let FontStylesComponent = {
  ...
  controller: [
    ...
    function (c) {
      ...
      c.select = function (a) {
        if (c.states[a]) delete c.states[a];
        else c.states[a] = a;
        ...
        if (c.isExpression) {
          ...
          if (
            valueRef &&
            valueRef.qStringExpression &&
            valueRef.qStringExpression.qExpr
          ) {
            valueRef.qStringExpression.qExpr += value;
          } else setRefValue(c.data, c.definition.ref, value);
        } else setRefValue(c.data, c.definition.ref, value);

        "function" == typeof c.definition.change &&
          c.definition.change(c.data, c.args.handler);
  ...
  <div class="pp-component" ng-if="visible">
        ...
        <div class="value">
          <div class="qv-object-qsstatistic" ng-if="!loading">
            <textarea rows="5" ng-model="t.value" ng-change="onTextChange()" ng-blur="onBlur()" style="width: 100%; max-width: 100%;">
            </textarea>
          </div>
        </div>
        <div class="pp-loading-container" ng-if="loading">
...
let SelectIconDialogComponent = function (ShowService) {
  return DialogComponentFactory(
    ...
    <div class="qv-object-qsstatistic">
      ...
      <div style="overflow:auto; -webkit-overflow-scrolling:touch; height:${
        docHeight / 2
      }px; border: solid 1px #f2f2f2;border-radius:5px;padding:5px">
